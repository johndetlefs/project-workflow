...
Invocation contract (Claude Code): supply values such as `<taskId>` or `<scope>` in the user request or current conversation. Treat angle-bracket values as required request fields, not literal text.
