...
2. Scaffold: use `project-scaffold` to create one task folder under `.project-workflow/tasks/<ID>-<Suffix>/`, with `REQUIREMENTS.md`, `IMPLEMENTATION.md`, and a tracker row.
...
- If the user asks to create a task, story, feature folder, tracker row, or new project-workflow item, use `.agents/skills/project-scaffold/SKILL.md`.
...
- If a task-specific workflow is requested but the task folder does not exist, run `project-scaffold` first before requirements, planning, clarification, implementation, review, or retro.
...
- If a task folder does not exist, run `./.project-workflow/cli/workflow task init --id <ID> --title "<TITLE>" --update-tracker` from the repo root.
