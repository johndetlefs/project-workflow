...
- `project init` must scaffold:
  ...
  - `AGENTS.md` and `.agents/skills/project-*` when Codex assets are selected
  - `.cursor/rules/project-workflow.mdc` when Cursor assets are selected
...
- If you add, remove, or rename a workflow step, update all matching assets: packaged prompts, Codex skills, Codex AGENTS template, Cursor rule, README, and the `project init` install lists.
