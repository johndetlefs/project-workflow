...
This runbook governs public releases. The current release is `0.3.0`; all commands and identities
...
python scripts/release_contract.py check-source --version 0.3.0 --tag v0.3.0 --clean
...
```bash
git tag --annotate v0.3.0 --message "project-workflow 0.3.0"
...
```bash
uvx --from project-workflow==0.3.0 project --version
python scripts/verify_package_journeys.py --from project-workflow==0.3.0 --version 0.3.0
gh attestation verify project_workflow-0.3.0-py3-none-any.whl --repo johndetlefs/project-workflow
```
...
- Never move or reuse a public tag or version. PyPI files are immutable and versions cannot be
  overwritten. If PyPI accepts `0.3.0` but a later step fails, preserve `v0.3.0`, repair the GitHub
