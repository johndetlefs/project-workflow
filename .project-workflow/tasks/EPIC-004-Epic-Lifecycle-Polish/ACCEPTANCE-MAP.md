...
| AC4 | A minimal epic status lifecycle exists for global epic rows where useful, with CLI-supported transitions or validation for `Analysing`, `Ready`, `In Progress`, `Closeout`, and `Complete`; transitions must not bypass readiness, audit, retro, or closeout gates. | TASK-018 (Complete) | TASK-018: parent AC evidence recorded; TASK-018: QA pass | None | Satisfied |
| AC5 | Doctor or validation output separates legacy/historical warnings from current actionable issues, with tests proving old APP/EPIC-002-style artifacts remain visible but do not obscure EPIC-003-or-newer gate failures. | TASK-019 (Complete) | TASK-019: parent AC evidence recorded; TASK-019: QA pass | None | Satisfied |
