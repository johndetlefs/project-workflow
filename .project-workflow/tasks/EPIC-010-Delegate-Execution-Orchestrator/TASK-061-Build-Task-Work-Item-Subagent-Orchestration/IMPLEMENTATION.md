...
| --: | ----- | ----------- | ------------------- | ----------------- | ------ |
| 1 | Task Executor And Packet | Implement Task-mode executor selection and bounded packet construction in the standalone/mirrored CLI architecture; TASK-060 graph/state APIs are read-only dependencies. | AC1, AC2 | Run selector/packet matrix and inspect exact payload fields. | To Do |
| 2 | Coordinator Integration Gate | Implement result verification, collision/scope checks, dependency release, and coordinator-only shared-state transitions in the Task-mode module. Depends on row 1. | AC3, AC4 | Exercise accepted and rejected worker results and compare shared-artifact hashes. | To Do |
| 3 | Failure And Resume Semantics | Implement descendant blocking, shared-premise halt, in-flight checkpoints, retry, resume, and orphan classification. Depends on row 2. | AC5 | Run failure/interruption state-transition matrix. | To Do |
| 4 | Testing Lifecycle Gate | Harden Task status so Testing requires every required implementation row Done and cannot be bypassed by ordinary force. | AC3 | Demonstrate rejected incomplete transition with unchanged state, then successful all-Done transition. | To Do |
| 5 | Live Task-Mode Journey | Execute the exact AC18 current-Codex journey and retain sanitised runtime-target-source evidence. Depends on rows 1-4. | AC6 | Inspect source/runtime identity, worker events, scopes, hashes, dependency timing, failure correction, and lifecycle results. | To Do |
| 6 | Child QA Handoff | Run focused/full tests, Doctor/build/privacy checks, record evidence, and submit to independent QA. Depends on row 5. | AC1, AC2, AC3, AC4, AC5, AC6 | Review commands, results, evidence recipe, and proof limitations. | To Do |
