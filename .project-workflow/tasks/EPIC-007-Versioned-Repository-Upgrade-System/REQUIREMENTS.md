...
- Approval note / source: Owner message on 2026-07-21: happy to completely go with the recommended Versioned Repository Upgrade System; preserve displaced scope in the backlog.
- Approved artifact identity: sha256:000871968973a2b1cb829681c0ca436d86efb449c585cd680badcf199c9d37db

## Goal

Make every supported project-workflow repository deterministically upgradeable to the current repository contract.

The owner-facing outcome is a trustworthy separation of responsibilities:
...
- `project upgrade` plans versioned repository-state transformations by default and applies them only through an explicit `--apply` action.
...
- Replacing `project init` as the canonical acquisition and managed-asset refresh path.
...
- Keep `project init` responsible for managed-asset installation and refresh; when repository state is behind, init must report the detected schema and direct the user or agent to the upgrade workflow without falsely claiming repository-state completion.
...
- Make `project upgrade` non-mutating by default and produce an ordered plan containing current and target versions, migration IDs, exact target files, expected transformations, preconditions, blockers, and owner decisions.
- Provide machine-readable upgrade-plan output suitable for agents and CI alongside concise human-readable output.
- Require explicit `project upgrade --apply` to execute a plan.
...
- AC3: Running `project upgrade` against each supported historical fixture produces an ordered, machine-readable and human-readable plan with current/target versions, migration IDs, exact target files, preconditions, blockers, and owner decisions, and causes no repository file changes.
...
- AC5: Canonical `project init` refreshes managed assets, reports detected asset/schema state, and directs behind repositories to `project upgrade` without claiming that user-owned repository state was migrated.
...
- Preserve the separation between init, doctor, and upgrade: install/refresh, diagnose, and transform.
...
- Default `project upgrade` to a non-mutating plan and require explicit `--apply`.
...
- Verify canonical init refreshes managed assets while accurately reporting schema state and directing upgrade.
...
| Integrate Init Version Detection And Upgrade Direction | AC1, AC5 | Keep canonical init honest about managed-asset refresh versus repository-schema state. |
...
| Align Documentation And Generated Agent Assets | AC2, AC3, AC5, AC9 | Document the enterprise upgrade contract and preserve packaged/generated behavioral parity. |
