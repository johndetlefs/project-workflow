...
| AC3 | Running `project upgrade` against each supported historical fixture produces an ordered, machine-readable and human-readable plan with current/target versions, migration IDs, exact target files, preconditions, blockers, and owner decisions, and causes no repository file changes. | TASK-042 (Complete), TASK-045 (Complete), TASK-046 (Proposed) | None | None | Mapped - evidence pending |
