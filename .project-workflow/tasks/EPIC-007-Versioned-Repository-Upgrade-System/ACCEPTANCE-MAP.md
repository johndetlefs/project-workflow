...
| AC2 | Doctor can emit stable machine-readable findings containing a code, severity, affected artifact, remediation ownership, and mechanical-upgrade eligibility, while preserving concise human-readable output. | TASK-041 (Complete), TASK-046 (Complete) | None | None | Mapped - evidence pending |
| AC3 | Running `project upgrade` against each supported historical fixture produces an ordered, machine-readable and human-readable plan with current/target versions, migration IDs, exact target files, preconditions, blockers, and owner decisions, and causes no repository file changes. | TASK-042 (Complete), TASK-045 (Complete), TASK-046 (Complete) | None | None | Mapped - evidence pending |
