...
| AC3 | Running `project upgrade` against each supported historical fixture produces an ordered, machine-readable and human-readable plan with current/target versions, migration IDs, exact target files, preconditions, blockers, and owner decisions, and causes no repository file changes. | TASK-042 (Approved), TASK-045 (Proposed), TASK-046 (Proposed) | None | None | Mapped - evidence pending |
| AC4 | `project upgrade --apply` applies exactly the validated mechanical plan only when worktree, version, and input-hash preconditions still match; dirty, stale, unsupported, or unrecognized state is rejected without mutation. | TASK-042 (Approved), TASK-043 (Proposed) | None | None | Mapped - evidence pending |
| AC5 | Canonical `project init` refreshes managed assets, reports detected asset/schema state, and directs behind repositories to `project upgrade` without claiming that user-owned repository state was migrated. | TASK-040 (Complete), TASK-041 (Complete), TASK-044 (Proposed), TASK-046 (Proposed) | None | None | Mapped - evidence pending |
...
| AC8 | Missing approvals, stale evidence, accepted warnings, and unresolved product decisions remain visible as named owner-owned follow-up and are never automatically converted into passing evidence or authority. | TASK-041 (Complete), TASK-042 (Approved), TASK-043 (Proposed), TASK-045 (Proposed) | None | None | Mapped - evidence pending |
