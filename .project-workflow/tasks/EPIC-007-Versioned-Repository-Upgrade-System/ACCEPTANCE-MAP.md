...
| AC4 | `project upgrade --apply` applies exactly the validated mechanical plan only when worktree, version, and input-hash preconditions still match; dirty, stale, unsupported, or unrecognized state is rejected without mutation. | TASK-042 (Complete), TASK-043 (Proposed) | None | None | Mapped - evidence pending |
...
| AC8 | Missing approvals, stale evidence, accepted warnings, and unresolved product decisions remain visible as named owner-owned follow-up and are never automatically converted into passing evidence or authority. | TASK-041 (Complete), TASK-042 (Complete), TASK-043 (Proposed), TASK-045 (Proposed) | None | None | Mapped - evidence pending |
