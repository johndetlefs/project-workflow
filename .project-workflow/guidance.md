...
- Verify a newly published package from an isolated directory with `uvx --no-cache --no-config --isolated` so repository-local UV configuration or a stale negative cache cannot masquerade as a missing public version. When registry surfaces disagree, inspect PyPI JSON and the simple index before diagnosing publication.
- Treat the release workflow's attested public wheel and sdist as the publication identities. A retained pre-merge candidate may have equivalent source and behavior but a different archive hash after the tagged release build; record both layers and validate the exact public files instead of claiming they are byte-identical.
